...
SELECT p.title, p.full_text, p.client_id, c.login
FROM posts p, clients c
WHERE p.client_id = c.id;


-- №3.	Выполните поиск по публикациям, автором котоырх является клиент "Mikle".
SELECT p.title, p.client_id, c.login
FROM posts p, clients c
WHERE p.client_id = c.id AND c.login LIKE "Mikle";
